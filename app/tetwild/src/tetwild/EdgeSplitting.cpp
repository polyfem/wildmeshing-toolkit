...
struct SplitEdge : public ::wmtk::SplitEdge
{
    ...
    SplitEdge(const wmtk::TetMesh& m_, tetwild::TetWild& m_app_)
        : wmtk::SplitEdge(m_)
        , m_app(m_app_){};

    bool before(const wmtk::TetMesh::Tuple& loc0)
    {
        auto& m = static_cast<const tetwild::TetWild&>(this->m);
        split_cache.local().changed_faces.clear();

        split_cache.local().v1_id = loc0.vid(m);
        auto loc1 = loc0.switch_vertex(m);
        split_cache.local().v2_id = loc1.vid(m);
        //
        size_t v1_id = split_cache.local().v1_id;
        size_t v2_id = split_cache.local().v2_id;

        split_cache.local().is_edge_on_surface = m.is_edge_on_surface(loc0);
    ...
    bool after(const std::vector<wmtk::TetMesh::Tuple>& locs)
    {
        size_t v_id = ux;
        ...
        if (!m_app.m_vertex_attribute[v_id].m_is_rounded) {
            ...
            m_app.m_vertex_attribute[v_id].m_pos =
                to_rational(m_app.m_vertex_attribute[v_id].m_posf);
        ...
        // surface
        m_app.m_vertex_attribute[v_id].m_is_on_surface = split_cache.local().is_edge_on_surface;
